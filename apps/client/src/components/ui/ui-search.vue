...
<template>
  <div class="search">
    <input
      v-model="value"
      class="search-input"
      :class="{ 'search-input--filled': value.length > 0 }"
      type="text"
    />
    ...
    <span class="clear" @click="$emit('update:modelValue', '')">&#215;</span>
  </div>
</template>
...
.search-input--filled ~ .clear {
  display: block;
}

.search-input:focus + .placeholder,
.search-input--filled + .placeholder {
  transform: translate(0, -50%);
  left: 24px;
}

.search-input--filled + .placeholder span {
  display: none;
}
</style>
